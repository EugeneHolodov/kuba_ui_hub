...
import '../widgets/kuba_nav_bar_tabs.dart';
import '../widgets/kuba_search_bar_bottom.dart';
import '../models/activity_item.dart';
import '../data/activity_data.dart';

class NavBarTabsOverviewPage extends StatefulWidget {
  const NavBarTabsOverviewPage({super.key});

  @override
  State<NavBarTabsOverviewPage> createState() => _NavBarTabsOverviewPageState();
}

class _NavBarTabsOverviewPageState extends State<NavBarTabsOverviewPage> {
  int _selectedTabIndex = 0;
  String? _searchQuery;

  // Define tabs
  final List<NavBarTab> _tabs = const [
    NavBarTab(label: 'New', icon: Icons.new_releases, index: 0),
    NavBarTab(label: 'Open', icon: Icons.lock_open, index: 1),
    NavBarTab(label: 'Done', icon: Icons.check_circle, index: 2),
    NavBarTab(label: 'Rejected', icon: Icons.cancel, index: 3),
  ];
  ...
  Widget build(BuildContext context) {
    final currentActivities = _getActivitiesForTab(_selectedTabIndex);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Nav Bar Tabs - Overview'),
        centerTitle: true,
      ),
      body: Column(
        children: [
